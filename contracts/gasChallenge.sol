...
contract gasChallenge {
    ...
    function optimizedFunction() public {
        uint sum = 0; // Caching the state variable

        unchecked { // Using unchecked block for arithmetic
            for (uint i = numbers.length; i > 0; i--) { // Using postfix decrement to iterate in reverse
                sum += numbers[i - 1];
                numbers[i - 1] = 0; // Resetting each element to 0
        }
    }
}
}